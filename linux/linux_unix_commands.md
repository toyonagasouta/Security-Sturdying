...
rm file         # ファイル削除
rm -rf dir      # ディレクトリを強制(-f:force)・再帰(-r:recursive)削除(注意して使用):-rを付与する事でディレクトリの中身を消してくれる。
cp src dst      # ファイルのコピー
cp -r src dst   # ディレクトリを再帰コピー：-rを付与する事でディレクトリの中身を再帰的に辿りコピーする。
mv src dst      # 移動・リネーム
